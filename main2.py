...
def downloadProgress(stream, chunk, bytes_remaining):
    ...
    progress = tkinter.ttk.Progressbar(download_frame,length=400,maximum=size,value=size-bytes_remaining)
    progress.update()

def downloadComplete(stream,file_path):
    setStatus("Downloaded in "+file_path)
...
def setDetails(yt):
    ...
    Button(details_frame,text="Download",bg="green",fg="white",command=lambda:downloadVideo(yt)).grid(row=3,column=0,columnspan=3)
